      ...
      {allData.length === 0 ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            <InsightList title="Mais Reversões" icon="🏆" type="best">
              {insights.topDrivers.map((d, i) => (
                <CompactHighlight 
                  key={i} name={d.name} 
                  metric={`${((d.revertidos/(d.totalTickets||1))*100).toFixed(0)}%`} 
            ...
            <InsightList title="Menos Reversões" icon="⚠️" type="worst">
            ...
            <InsightList title="Maior Volume" icon="📦" type="best">
              {insights.topVolumeDrivers.map((d, i) => (
                <CompactHighlight 
                  key={i} name={d.name} 
                  metric={`${d.totalTickets}`} 
                  detail={formatCurrency(d.totalValue)}
                  type="best"
                  route={d.routes?.[0]}
                  onRouteClick={handleRouteClick}
                  onClick={() => handleDriverClick(d.name)}
            ...
            <InsightList title="Top Rotas (Rev%)" icon="📍" type="worst">
              {insights.topRoutes.map((r, i) => (
                <CompactHighlight 
                  key={i} name={r.locationName}
                  metric={`${((r.revertidos/(r.totalTickets||1))*100).toFixed(0)}%`}
                  detail={`${r.totalTickets} tks · ${r.drivers.size} mot`}
                  type="worst"
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-8">
            <div className="bg-white rounded-2xl md:rounded-3xl shadow-lg overflow-hidden border border-gray-100">
              <div className="p-4 md:p-6 border-b border-gray-100 bg-gradient-to-r from-emerald-600 to-teal-500 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <h3 className="text-base md:text-xl font-black uppercase tracking-tight">Rotas</h3>
                <input type="text" placeholder="Filtrar rota..." className="px-4 py-2 bg-white/10 border border-white/20 rounded-xl text-xs md:text-sm text-white outline-none w-full sm:w-48 placeholder-white/50" onChange={(e) => setRouteSearch(e.target.value)} />
              </div>
              <div className="overflow-x-auto max-h-[400px] md:max-h-[500px] overflow-y-auto">
                <table className="w-full text-left border-collapse min-w-[300px] md:min-w-0">
                  <thead className="bg-gray-100 text-gray-500 text-[8px] md:text-[9px] font-black uppercase tracking-widest border-b border-gray-200 sticky top-0">
                    <tr>
                      <th className="px-4 py-3 md:px-5 md:py-4">Rota</th>
                      <th className="px-4 py-3 md:px-5 md:py-4 text-right">Rev%</th>
                    </tr>
                  </thead>
